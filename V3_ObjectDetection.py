...
class AdaptiveObjectDetector:
    def __init__(self, base_path=None):
        # Set base path to script directory if not specified
    ...
    def classify_object(self, image_crop, bbox):
        ...
        # Auto-save database after every update
        self.save_database()
    ...
    def save_object_image(self, image_crop, folder_name):
        """Save cropped object image to its folder in script directory"""
        folder_path = os.path.join(self.base_path, folder_name)
        os.makedirs(folder_path, exist_ok=True)
                            ...
                            for folder in current_folders:
                                if folder not in [d['folder_name'] for d in self.object_database.values()]:
                                    ...
                                    if os.path.exists(folder_path) and len(os.listdir(folder_path)) > 0:
                                        self.folder_mappings[object_id] = folder
                                        # Save database when folder mapping changes
                                        self.save_database()
                                        break
    ...
    def load_database(self):
        """Load object database from script directory"""
    ...
    def save_database(self):
        """Save object database to script directory"""
        db_path = os.path.join(self.base_path, "object_database.json")
        try:
    ...
    def run_detection(self):
        ...
        print("Press 'q' to quit - database auto-saves continuously")
